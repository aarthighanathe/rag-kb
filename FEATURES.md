...
4a. **Full feature grid** ("EVERYTHING ELSE IN THE ARCHIVE") — not a grid at all: a masonry "pinned stamp board" (1/2/3 CSS columns, `break-inside: avoid`) covering the features not already called out elsewhere on the page: hybrid search & re-ranking, document relationship map, auto-tagging, duplicate detection, answer feedback, one-click re-query, content-aware suggestions, searchable query history, full keyboard control, prompt-injection filtering, per-account isolation, and magic-byte file validation. Each card has a natural (unequal) height, a small deterministic alternating tilt (±0.5°–1.5° per index, fixed so it never shifts on re-render), a colored "pin" dot at its top edge, an inline icon + display-italic title, one sentence of body copy, and a mono "FILE No. 0X" tag. On hover/focus a card un-tilts to 0°, lifts a few pixels with a deeper shadow, and its pin + icon recolor from archive-green to stamp-red — reusing the same raw-DOM `onMouseEnter`/`onMouseLeave` transition pattern as the hero CTA elsewhere on the page.
