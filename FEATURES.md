...
- On success, status is set to `ready` (via `updateChunkCount`, then redundantly again via `updateDocumentStatus`), and the staged upload file is deleted from Storage.
...
Index: `idx_chunks_embedding_cosine` — IVFFlat (`vector_cosine_ops`, `lists = 100`), chosen over HNSW for lower memory footprint on Supabase's free/pro tiers and faster O(n) build time at the project's expected scale (<100k chunks). A GIN trigram index (`idx_chunks_content_trgm`) also exists on chunk content and backs `hybridSearch`'s pg_trgm keyword-search leg (§3.1).
...
How it works:
- `POST /api/query` validates the request body (`query` 3–1000 chars, optional `documentIds` up to 10, `matchCount` 1–10 default 5, `similarityThreshold` 0..1 default 0, `history` up to 6 turns) and stores it in an in-memory map keyed by a generated `queryId`, with a 2-minute TTL.
...
How it works:
- Before embedding, `rewriteQueryForRetrieval()` (`services/queryRewriter.ts`) can send the sanitized query through one extra Groq call (`llama-3.1-8b-instant`) that reformulates it into a short, passage-like statement (HyDE-lite) — compensating for `all-MiniLM-L6-v2`'s documented weakness at asymmetric question/passage retrieval.
...
How it works:
- After an answer has fully streamed to the client and the SSE `complete` event has already been sent, `validateAnswer()` (`services/answerValidator.ts`) fires a second, fire-and-forget Groq call (`llama-3.1-8b-instant` — the same fast model used for generation, not a heavier model, since this runs in the background and isn't worth the extra latency/cost) that checks the completed answer against the chunks it was built from for hallucinations, contradictions, unsupported claims, missing citations, and internal inconsistencies.
...
- `audit_logs` schema (`supabase/migrations/008_add_audit_logs.sql`): `operation` (CHECK-constrained to the 3 real operation types), `user_id`, `resource_id`, `resource_type`, `success`, `details` (JSONB), `correlation_id`, `created_at`. RLS follows the same two-layer convention as every other table (`service_role` bypass for the backend, a `SELECT`-only `authenticated`-scoped policy for the row's own owner).
...
### 10.5 Manual Backups & Data Retention
Status: ✅ (manual/on-demand — no scheduler)

How it works:
- `npm run backup:full` / `backup:incremental` / `backup:list` / `backup:restore -- --file <path>` / `backup:cleanup` (`backend/scripts/backup.ts`, dispatching into `services/backupService.ts`) back up `documents`, `document_chunks`, `query_logs`, and `audit_logs`. Full backups prefer `pg_dump` (via `DATABASE_URL`) when available, falling back to a Supabase-API row export per table if `pg_dump` isn't installed on the host.
- These are manual/on-demand commands, not an automated schedule — this project has no long-running always-on process suited to a cron-style trigger yet (a free-tier host that sleeps on idle can't reliably fire a scheduled job), so the commands above are real and runnable, but backups only happen when someone runs them.
- `services/dataRetention.ts` provides `cleanupExpiredQueries()`/`cleanupExpiredAuditLogs()` (query logs retained 1 year, audit logs 2 years) and `getExpiringDataSummary()`, scoped to the two tables that actually exist with a relevant timestamp — an earlier draft also referenced a `documents.deleted_at` soft-delete column and an `upload_queue` table, neither of which exist (documents are hard-deleted immediately; upload state lives entirely in BullMQ). Like backups, these cleanup functions are callable but not yet wired to any scheduler — they'd need the same kind of on-demand or cron trigger.
...
| No working dark mode — `ThemeContext.tsx` exists (localStorage persistence, system-preference detection, sets `data-theme` on `<html>`) but is not mounted anywhere, and mounting it alone would not be sufficient: the entire color system is hardcoded static hex values in `tailwind.config.js` (`ds.base`, `ds.text.primary`, etc.) with no CSS-custom-property indirection and no Tailwind `dark:` variant strategy — nothing in the CSS actually reads `data-theme`. This is a real design-system conversion (define color tokens as CSS variables, redefine them under a dark selector, then audit every component using `ds-*` classes for a dark-appropriate variant), not a wiring task | `frontend/src/contexts/ThemeContext.tsx`, `frontend/tailwind.config.js` | Med | Convert the Tailwind color tokens to CSS variables before attempting to mount `ThemeProvider` or add a toggle control |
