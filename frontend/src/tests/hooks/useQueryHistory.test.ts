...
describe('useQueryHistory', () => {
  ...
  it('loads existing entries from localStorage', () => {
    localStorage.setItem(
      'lumina:query-history',
      JSON.stringify([{ id: 'existing', ...makeEntry('existing query') }]),
    );
    const { result } = renderHook(() => useQueryHistory());
  ...
  it('add() appends entry and updates state', () => {
    const { result } = renderHook(() => useQueryHistory());
    act(() => {
      result.current.add(makeEntry('new query'));
    });
    expect(result.current.entries).toHaveLength(1);
  ...
  it('remove() removes entry by id', () => {
    const { result } = renderHook(() => useQueryHistory());
    act(() => {
      result.current.add(makeEntry('keep'));
    });
    act(() => {
      result.current.add(makeEntry('remove'));
    });
  ...
  it('clear() removes all entries', () => {
    const { result } = renderHook(() => useQueryHistory());
    act(() => {
      result.current.add(makeEntry('a'));
    });
    act(() => {
      result.current.add(makeEntry('b'));
    });
    act(() => {
  ...
  it('entries are sorted most-recent-first', () => {
    const { result } = renderHook(() => useQueryHistory());
    act(() => {
      result.current.add(makeEntry('first'));
    });
    act(() => {
      result.current.add(makeEntry('second'));
    });
  ...
  it('persists to localStorage after add', () => {
    const { result } = renderHook(() => useQueryHistory());
    act(() => {
      result.current.add(makeEntry('persisted'));
    });
    const stored = JSON.parse(localStorage.getItem('lumina:query-history')!);
